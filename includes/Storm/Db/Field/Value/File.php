...
class Storm_Db_Field_Value_File extends Storm_Db_Field_Value {
	...
	*/
	function __construct( array $params = array() ) {
		if( ! $params['path'] ) {
			throw new Storm_Exception( "Необходимо указать параметр 'path' — путь к каталогу для хранения файлов." );
		}
	...
	protected function loadValue( $value ) {
		...
		if( is_array( $value ) && array_key_exists( 'error', $value ) && array_key_exists( 'name', $value ) && array_key_exists( 'tmp_name', $value ) ) {
			if( $value['error'] == UPLOAD_ERR_OK ) {
				...
				switch( $value['error'] ) {
					case UPLOAD_ERR_INI_SIZE:
						throw new Storm_Exception( "Файл слишком велик. Максимальный размер файла — ".ini_get( 'upload_max_filesize' )."." );
						break;
					case UPLOAD_ERR_FORM_SIZE:
						throw new Storm_Exception( "Файл слишком велик." );
						break;
					case UPLOAD_ERR_PARTIAL:
						throw new Storm_Exception( "Файл не был полностью загружен. Повторите загрузку." );
						break;
					case UPLOAD_ERR_NO_TMP_DIR:
						throw new Storm_Exception( "Отсутствует временный каталог для записи файла. Обратитесь в службу технической поддержки." );
						break;
					case UPLOAD_ERR_CANT_WRITE:
						throw new Storm_Exception( "Не удалось записать файл на диск. Обратитесь в службу технической поддержки." );
						break;
					case UPLOAD_ERR_CANT_WRITE:
						throw new Storm_Exception( "Не удалось записать файл на диск. Обратитесь в службу технической поддержки." );
						break;
					case UPLOAD_ERR_EXTENSION:
						throw new Storm_Exception( "Недопустимый тип файла. Используйте другой файл." );
						break;
					default: 
						throw new Storm_Exception( "При загрузке файла произошла неизвестная ошибка. Попробуйте загрузить файл заново или используйте другой файл. Обратитесь в службу технической поддержки, если ошибка не исчезает." );
						break;
				}
			}
			...
			if( substr_compare( $rootRealPath, $fileRealPath, 0, strlen( $rootRealPath ) ) == 0 ) {
				...
				// Отаке детектед!
				throw new Storm_Exception( "Загрузка файла {$value} запрещена из соображений безопасности." );
			}
			...
			throw new Storm_Exception_Validation( "Недопустимое значение для загрузки файла: '{$value}'." );
		}
	}
	...
	function loadSource( $source ) {
		...
		if( ! @file_exists( $path ) ) {
			if(! @mkdir( $path, 0755, true ) ) {
				throw new Storm_Exception( "Невозможно создать каталог '{$path}' для сохранения файла." );
			}
		} else if( ! is_writeable( $path ) ) {
			throw new Storm_Exception( "Невозможно сохранить файл, запись в каталог '{$path}' запрещена." );
		}
		
		// Каталоги существуют и доступны для записи. Самое время попробовать прочитать данные.
		$data = file_get_contents( $source['path'] );
		if( $data === false ) {
			throw new Storm_Exception( "Невозможно прочитать файл '{$source['path']}'." );
		}
		...
		if( $written === false || $written != strlen( $data ) ) {
			throw new Storm_Exception( "Произошла ошибка при записи файла '{$path}/{$filename}'." );
		}
	...
	*/
	...
	function beforeSync() {
		...
		if( ! @file_exists( $path ) ) {
			if(! @mkdir( $path, 0755, true ) ) {
				throw new Storm_Exception( "Не удалось создать каталог хранения файлов '{$path}'. Недостаточно привилегий доступа." );
			}
		} else if( ! is_writeable( $path ) && !( @chmod( $path, 0755 ) && is_writeable( $path ) ) ) {
			throw new Storm_Exception( "Запись в каталог '{$path}' запрещена." );
		}
    }
}
