...
class StormFlvDbFieldValue extends StormFileDbFieldValue {
	...
	function __construct( array $params = array() ) {
		...
		if( ! $params['ffmpeg'] ) {
			throw new StormException( "Необходимо указать параметр 'ffmpeg' — путь к утилите ffmpeg для преобразования видеофайлов." );
		}
	...
	protected function loadValue( $value ) {
		...
		if( $this->source ) {
			...
			exec( StormUtilities::array_printf( $convertCmd, $options ), $output, $retcode );
	...
	function beforeSave() {
		...
		if( ! is_null( $this->value ) ) {
			...
			exec( StormUtilities::array_printf( $screenshotCmd, $options ), $output, $retcode );
