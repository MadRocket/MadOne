...
class StormRestProcessor
{
     ...
     * @param $uri - request uri, ie '/5/update/'
     * @param $vars - request params (usually Mad::vars())
        ...
        try {
            ...
            elseif (array_key_exists(0, $names)) {
                ...
                switch ($names[0]) {
                    ...
                    case 'create':
                        $data = StormQuerySet($model)->create((count($vars) == 1 && array_key_exists('json_data', $vars))
                                                                      ? json_decode($vars['json_data'], true)
                                                                      : $vars)->asArray();
                        break;
                    ...
                    default:
                        ...
                        if (is_numeric($names[0])) {
                            switch ($names[1]) {
                                case 'update':
                                    $data = $this->getModelObject($model, $names[0])->copyFrom((count($vars) == 1 && array_key_exists('json_data', $vars))
                                                                                                       ? json_decode($vars['json_data'], true)
                                                                                                       : $vars)->save()->asArray();
                                    break;
