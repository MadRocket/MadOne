...
/**
	Поле — внешний ключ
*/
class StormFkDbFieldValue extends StormDbFieldValue {
	...
	function set( $value ) {
		...
		if( is_object( $value ) ) {
			if( ! $value instanceof $this->model ) {
				throw new StormValidationException( "Cannot use object of '". get_class( $value ) ."' class as a  foreign key to '{$this->model}' value" );
			}
			$this->instance = $value;
		...
		throw new StormValidationException( "Cannot use '{$value}' as a foreign key field value" );
	}
