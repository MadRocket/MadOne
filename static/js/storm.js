...
Storm.buildPath = function ( element ) {
	...
	path.model	= nesting.filter( '[stormModel]' ).attr( 'stormModel' );
	path.object	= nesting.filter( '[stormObject]' ).attr( 'stormObject' );
	path.field	= nesting.filter( '[stormField]' ).attr( 'stormField' );
...
Storm.Form.show = function () {
    ...
    this.form.find('.tabs > li > a').click(function(){
        alert(1);
        $(this).parent('.tabs').find('li').removeClass('active');
