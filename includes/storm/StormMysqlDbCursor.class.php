...
class StormMysqlDbCursor extends StormDbCursor
{
    private $result = NULL;
    ...
    */
    function execute( $query, array $args = array() )
    {
        ...
        foreach( $args as $key => $val )
        {
        	if(is_array($val)) {
				...
				foreach($val as $k => $v) {
					$escaped[] = "'" . $this->connection->escape( ( is_object( $v ) && $v instanceof StormModel ) ? $v->meta->getPkValue() : $v  ) . "'";
				}
        ...
        $filled_query = StormUtilities::array_printf( $query, $args );

        if( StormConfig::$db_debug )
        {
    ...
    private function checkResult()
    {
        ...
        if( ! $this->result )
        {
            throw new StormException( 'A query that selects some rows must be executed before any data could be fetched' );
        }
        return true;
    }
    ...
    function scroll( $offset, $mode = 'rel' )
    {
        ...
        {
            throw new StormException( "Must specify scroll mode with 'abs' or 'rel' value, not '{$mode}'" );
        }
