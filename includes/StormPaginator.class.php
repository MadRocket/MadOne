...
class StormPaginator
{
    ...
    public function fetch()
    {
        ...
        if( $pcnt > 1 )
        {
            ...
            {
                $pg = array( 'title' => $i );
                if( $this->page != $i ) $pg['uri'] = StormUtilities::array_printf( $this->uri, array( 'page' => $i ) );
                $this->pages[] = $pg;
            }
            ...
            {
                $b1; $b2; $m1; $m2; $e1; $e2;  # Границы срезов - начало, середина, конец
                ...
                if( $use_m ) $p2 = array_slice( $this->pages, $m1, $m2 - $m1 + 1 );
                $p3 = array_slice( $this->pages, $e1, $e2 - $e1 + 1 );

                if( $use_el1 ) $p1[] = array( 'title'=>'...', 'ellipsis'=>1 );
                if( $use_el2 ) array_unshift( $p3, array( 'title'=>'...', 'ellipsis'=>1 ) );
    ...
    */
    function __get( $name )
    {
        switch( $name )
        {
            ...
            case 'objects': return $this->getObjects();
            case 'pageCount': return ceil( $this->getCount() / $this->size );
        }
