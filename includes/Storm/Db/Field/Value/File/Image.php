...
class Storm_Db_Field_Value_File_Image extends Storm_Db_Field_Value_File {
	...
	function __construct( array $params = array() ) {
		...
		if( array_key_exists('variants', $params) && is_array( $params['variants'] ) ) {
			...
			if( array_key_exists( 'original', $params['variants'] ) ) {
				throw new Storm_Exception( "Использование варианта изображения 'original' запрещено, это название зарезервировано для исходного изображения." );
			}
	...
	protected function copyValue( $source ) {
		if( is_object( $source ) ) {
			...
			foreach( array_keys( $this->variants ) as $variant ) {
				$this->value->$variant = new Storm_Utilities_Imagevariant( $source->$variant );
			}
		}
	}
		...
		if( $this->value ) {
			...
			foreach( array_keys( $this->variants ) as $variant ) {
				if( ! property_exists( $this->value, $variant ) ) {
					$this->value->$variant = new Storm_Utilities_Imagevariant();
				}
			...
			$this->value->original = new Storm_Utilities_Imagevariant();
	...
	public function loadSource( $source ) {
		...
		foreach( array_merge( array( '' ), array_keys( $this->variants ) ) as $name ) {
			...
			if( ! @file_exists( $dir ) ) {
				if(! @mkdir( $dir, 0755, true ) ) {
					throw new Storm_Exception( "Невозможно создать каталог '{$dir}' для сохранения изображения." );
				}
			} else if ( ! is_writeable( $dir ) ) {
				throw new Storm_Exception( "Невозможно сохранить изображение: запись в каталог '{$dir}' запрещена." );
			}
		}
		...
		try
		{
			$value->original = new Storm_Utilities_Imagevariant();
			$value->original->uri = "{$this->path}/{$this->filename}";
			$value->original->selfCheck();
	
			if( ! ( $value->original->width && $value->original->height ) ) {
				throw new Storm_Exception( "Файл '{$this->source['name']}' не является изображением." );
			}

			$path = "{$_SERVER['DOCUMENT_ROOT']}{$this->path}";
			foreach( $this->variants as $name => $options ) {
				$v = new Storm_Utilities_Imagevariant();
	...
	function beforeSync() {
		...
		foreach( array_keys( $this->variants ) as $name ) {
			...
			if( ! @file_exists( $dir ) ) {
				if(! @mkdir( $dir, 0755, true ) ) {
					throw new Storm_Exception( "Не удалось создать каталог хранения изображений '{$dir}'. Недостаточно привилегий доступа." );
				}
			} else if( ! is_writeable( $dir ) && !( @chmod( $dir, 0755 ) && is_writeable( $dir ) ) ) {
				throw new Storm_Exception( "Запись в каталог '{$dir}' запрещена." );
			}
		}
    }
	...
	*/
	function checkImageFormat( $type ) {
		...
		if( ! array_key_exists( $type, $typeCheck ) ) {
			throw new Storm_Exception( "Изображения в формате ". image_type_to_extension( $type, false ). " не поддерживаются." );
		}
		
		if( ! imagetypes() & $typeCheck[ $type ] ) {
			throw new Storm_Exception( "Изображения в формате ". image_type_to_extension( $type, false ). " не поддерживаются графической библиотекой сайта." );
		}
	...
	*/
	function loadTransformOptions( $defaults, $overrides ) {
		...
		case 'png':
			...
			break;
		default:
			throw new Storm_Exception( "Формат {$opts['format']} неизвестен. Используйте jpeg, png или gif." );
			break;
		}
		...
		foreach( array( 'frame', 'background' ) as $k ) {
			$opts[ $k ] = is_null( $opts[ $k ] ) ? null : new Storm_Utilities_Rgb( $opts[ $k ] );
		}
	...
	function transformImage( $src, $dst, array $args ) {
		...
		try {
			...
			throw new Storm_Exception( "Ошибка изменения размеров изображения {$src}. ".$e->getMessage() );
		}
		
		// Проверим файл
		if( ! file_exists( $src ) ) {
			throw new Storm_Exception( "Файл {$src} не существует." );
		}
		if( ! is_readable( $src ) ) {
			throw new Storm_Exception( "Файл {$src} не доступен для чтения." );
		}
		...
		if( ! $oriW || ! $oriH ) {
			throw new Storm_Exception( "При чтении параметров изображения {$src} произошла ошибка. Файл не является изображением или поврежден." );
		}
		...
		*/
		...
		if( $watermark ) {
			...
			if( ! file_exists( $watermark ) ) {
				throw new Storm_Exception( "Файл водяного знака  {$watermark} не существует." );
			}
			if( ! is_readable( $watermark ) ) {
				throw new Storm_Exception( "Файл водяного знака {$watermark} недоступен для чтения." );
			}
			...
			if( ! $watW || ! $watH ) {
				throw new Storm_Exception( "При чтении параметров водяного знака {$watermark} произошла ошибка. Файл не является изображением или поврежден." );
			}
