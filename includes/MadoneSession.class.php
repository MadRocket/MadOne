...
class MadoneSession
{
    ...
    */    
    public static function getInstance() {
        if( self::$instance == null ) self::$instance = new MadoneSession();
    ...
    */
    public function login( $name, $password ) {
        ...
        if( $name != '' && $password != '' )
        {
            $this->user = MadoneUsers( array( 'login' => $name, 'password' => md5( $password ) ) )->follow( 1 )->first();
    ...
    */
    public function reloadData() {
        $this->user = MadoneUsers( array( 'login' => $this->user->login ) )->follow( 1 )->first();
        $this->session['user'] = $this->user ? serialize( $this->user ) : null;
    }
