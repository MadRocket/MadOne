...
abstract class StormKiModel extends StormModel
{
    ...
    */
    final function _definition()
    {
        return array
        (
            'lk'       => new StormIntegerDbField( array( 'null' => false, 'index' => 'Ki' ) ),
            'rk'       => new StormIntegerDbField( array( 'null' => false, 'index' => 'Ki' ) ),
            'lvl'      => new StormIntegerDbField( array( 'null' => false, 'index' => 'Ki' ) ),
        );
    }
    ...
    */
    function checkKiIntegrity()
    {
        ...
        try {
            ...
            if( count( $bad_keys ) ) throw new StormException( "Ki integrity check failed: got nodes with lk >= rk: ".join( ', ', $bad_keys )."." );
            ...
            if( $min_lk != 1 ) throw new StormException( "Ki integrity check failed, the least lk = {$min_lk} instead of 1." );
            ...
            if( $max_rk != $count * 2 ) throw new StormException( "Ki integrity check failed, the biggest rk = {$max_rk} instead of ".( $count * 2 )."." );
            ...
            $bad_keys = array_map( create_function( '$row', 'return $row["pk"];' ), $cursor->fetchAll() );
            if( count( $bad_keys ) ) throw new StormException( "Ki integrity check failed, there are nodes with even (rk - lk): ".join( ', ', $bad_keys )."." );
            ...
            $bad_keys = array_map( create_function( '$row', 'return $row["pk"];' ), $cursor->fetchAll() );
            if( count( $bad_keys ) ) throw new StormException( "Ki integrity check failed, there are nodes with odd (lk - lvl): ".join( ', ', $bad_keys )."." );
            ...
            $bad_keys = array_map( create_function( '$row', 'return $row[pk];' ), $cursor->fetchAll() );
            if( count( $bad_keys ) ) throw new StormException( "Ki integrity check failed, there are duplicated keys: ".join( ', ', $bad_keys )."." );
        }
        catch( StormException $e ) {
            // При проверке валидности обнаружена ошибка! Исправим индекс и выбросим exception заново :D
            $this->rebuildKiIndex();
            throw new StormException( $e->getMessage() . " Ki index was automatically rebuilt." );
        }
    ...
    */
    protected function moveAt( $new_place, $new_level )
    {
        ...
        if( $new_place >= $this->lk && $new_place <= $this->rk ) throw new StormException( "Cannot move node into itself" );
    ...
    */
    protected function getObject( $node )
    {
        if( ! is_object( $node ) )
        {
            $qs = new StormQuerySet( $this->meta->name );
        
            if( ! $record = $qs->get( $node ) )
            {
                throw new StormException( "Cannot get an anchor node with key='{$node}' for {$this->meta->name} model" );
            }
            $node = $record;
        }
        else if( ! $node instanceof $this->meta->name )
        {
            throw new StormException( "Cannot use ".get_class( $node )." class as an anchor for {$this->meta->name} model" );
        }
    ...
    */
    protected function putAsNeighbour( $neighbour, $place )
    {
        ...
        if( $neighbour->lvl == 1 ) throw new StormException( "Cannot put node ".( $place ? "after" : "before" )." a root node for {$this->meta->name} model" );
    ...
    */
    ...
    {
        return new StormKiQuerySet( $this->meta->name );
    }
    ...
    */
    ...
    {
        if( ! is_array( $this->_children ) ) throw new StormException( "Cannot set children of not children-enabled object"  );
        $this->_children[] = $node;
    }
    ...
    */
    ...
    {
        if( ! is_array( $this->_children ) ) throw new StormException( "Cannot get children of not children enabled object. Consider using ->tree() method of QuerySet to get objects with children."  );
