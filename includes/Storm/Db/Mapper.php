...
*/
abstract class Storm_Db_Mapper
{
    ...
    */
	function getColumnDefinitionSql( Storm_Db_Field $field, $language = null ) {
        // Получим класс поля
        $class = get_class( $field );

        if( ! array_key_exists( $class, $this->creation_data_types ) ) {
            throw new Storm_Exception( "Don't know how to map {$class} to database column type" );
        }
    ...
    *	$field — поле, объект — потомок Storm_Db_Field
    *	$language — язык, Storm_Language
    *	Возвращает строку - имя колонки в базе данных
    */
    function getFieldColumnName( Storm_Db_Field $field, $language = null ) {
    	if( ! $field->localized ) {
			return $field->name;
    	}
		if( ! $language ) {
	...
	*
	*/    
    function getFieldFulltextColumnName( Storm_Db_Field $field, $language = null ) {
		if( ! $language ) {
			$language = Storm_Core::getLanguage();
		}
		return	$field->name.
    ...
    */
    function getModelIndexes( $model )
    {
        ...
        foreach( $meta->getFields() as $name => $field )
        {
            ...
            // Primary key
            if( $field instanceof Storm_Db_Field_Auto )
            {
                $indexes[ $idx_name ] = new Storm_Index( $model, array( $name ) );
        	...
        	if( $gotLocalizedFields ) {
				foreach( Storm_Core::getAvailableLanguages() as $language ) {
					foreach( $fulltextFields as $field ) {
						$idx1 = new Storm_Index( $model, array( $this->getFieldColumnName( $field, $language ) ) );
						$idx1->fulltext = true;
						$indexes[] = $idx1;
						$idx2 = new Storm_Index( $model, array( $this->getFieldColumnName( $field, $language ), $this->getFieldFulltextColumnName( $field, $language ) ) );
						$idx2->fulltext = true;
						$indexes[] = $idx2;
					}
					$idx1 = new Storm_Index( $model );
					$idx1->fulltext = true;
					$idx2 = new Storm_Index( $model );
					$idx2->fulltext = true;
					foreach( $fulltextFields as $field ) {
        		...
        		foreach( $fulltextFields as $field ) {
					$idx1 = new Storm_Index( $model, array( $this->getFieldColumnName( $field ) ) );
					$idx1->fulltext = true;
					$indexes[] = $idx1;
					$idx2 = new Storm_Index( $model, array( $this->getFieldColumnName( $field ), $this->getFieldFulltextColumnName( $field ) ) );
					$idx2->fulltext = true;
					$indexes[] = $idx2;
        		}
				$idx1 = new Storm_Index( $model );
    ...
    */
    ...
    abstract function getTableList( Storm_Db_Cursor $cursor );
    abstract function getIndexCreationSql( Storm_Index $idx );
