...
class Storm_Queryset
{
    ...
    */
    function __construct( $model )
    {
        ...
        {
            throw new Storm_Exception( "$model is not a Storm model" );
        }
    ...
    */
    ...
    /**
        Объединение двух массивов объектов Storm_Query_Join.
    ...
    function getStormQueryParts( $type = 'values', array $extra = array())
    {
        ...
        switch( $type)
        {
            ...
            case 'values':
                ...
                $Limit = Storm_Core::getMapper()->getLimitOffsetSql( $extra['limit'], $extra['offset'] );
                ...
                if( count( $this->order ) > 0 )
                {
                    ...
                    foreach( $this->order as $o )
                    {
                        ...
                        {
                            $cf = new Storm_Db_Field_Complex( $this->model, $o['field'] );
                        ...
                        else
                        {
							...
							$field = Storm_Core::getInstance()->getStormModelMetadata( $this->model )->getField( $o['field'] );
                            $orders[] = "{$Alias}.". Storm_Core::getMapper()->getFieldColumnName( $field ) ." {$o['order']}";
                        }
                    }
            ...
            default: throw new Storm_Exception( "Unknown query type '{$type}'" );
        }
    ...
    */
    ...
    /**
        ...
        Возвращает массив объектов Storm_Query_Entity в нужном порядке
    */
    protected function getStormQueryEntities( $model = null, $depth = 0, $path = array(), $supalias = null, $null = false )
    {
        ...
        $part = new Storm_Query_Entity( $model ? $model : $this->model );
        ...
        if( $model )
        {
            ...
            $part->join = new Storm_Query_Join
            (
                $null ? 'LEFT' : 'INNER',
            ...
            {
                if( ! $v instanceof Storm_Db_Field_Fk || $depth == $this->follow )
                {
                	if( $v->localized ) {
                		foreach( Storm_Core::getAvailableLanguages() as $language ) {
							...
							$part->fields[] = "{$alias}." . Storm_Core::getMapper()->getFieldColumnName( $v, $language ) . " AS $falias";
							$part->aliases[ $n ][ $language->getName() ] = "$falias";
                		}
                	} else {
						$falias = join( '__', array_merge( array( $this->model ), $path, array( $n ) ) );
						$part->fields[] = "{$alias}." . Storm_Core::getMapper()->getFieldColumnName( $v ) . " AS $falias";
        ...
        else
        {
            ...
            foreach( $meta->getFields() as $n => $v )
            {
            	...
            	if( empty($this->onlyFields) || in_array($n, $this->onlyFields) || $v instanceof Storm_Db_Field_Auto ) {
        ...
        foreach( $meta->getFields() as $n => $v )
        {
            if( $v instanceof Storm_Db_Field_Fk )
            {
		...
		foreach( $backend->cursor->fetchAll() as $row )
        {
            ...
            foreach( $query['fields'] as $f )
            {
                ...
                if( $record )
                {
                    ...
                    $meta = Storm_Core::getInstance()->getStormModelMetadata( $f->model );
    ...
    */
    protected function getCacheKey( $params )
    {
        ...
        return $this->model . ':'. join( ',', $key ) . ':' . Storm_Core::getLanguage()->getKey();
    }
    ...
    function get( $params, $use_cache = true )
    {
        ...
        if( ! is_array( $params ) )
        {
            if( ! ( is_numeric( $params ) || $params instanceof Storm_Model ) )
                throw new Storm_Exception( 'Must use filter parameter, integer value or Storm_Model instance to get model object ' );
