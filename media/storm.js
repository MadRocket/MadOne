 ...
 * Принимает три вида аргументов. Каждый вызов use соответствует одной из функций серверного Storm_Queryset.
 * Примеры:
 ...
 * и так далее. По сути — интерфейс ко _всем_ методам Storm_Queryset. И результат — прям такой же, как получится на сервере,
		...
		for( var name in this.loadedData ) {
			if( this.loadedData.hasOwnProperty( name ) ) {
				...
				if( name in this.inputs ) {
					...
					// TODO добавить куда-то сюда обработку Storm_Db_Field_File_Image - они тоже приходят в виде объекта, и что-то с ним надо делать
				}
